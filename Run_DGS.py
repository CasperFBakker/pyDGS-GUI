...
def Store_Percentile(path_of_the_directory, Image_Name, Percentile, Description):          
        ...
        temp.to_csv('Output data/Bloemendaal/Percentiles/temp_percentile.csv', index=False)

        try: 
            DF = pd.read_csv("Output data/Bloemendaal/Percentile_" + dir_name + "_" + Description + ".csv")
            ...
            if Image_Name in DF.values:
                ...
                merged.to_csv("Output data/Bloemendaal/Percentile_" + dir_name + "_" + Description + ".csv", index=False)

        except FileNotFoundError:
            temp = pd.DataFrame([data], columns=[columns])
            temp.to_csv("Output data/Bloemendaal/Percentile_" + dir_name + "_" + Description + ".csv", index=False)
...
def Store_Percentage(path_of_the_directory, Image_Name, Percentage, Description):          
        ...
        try: 
            DF = pd.read_csv("Output data/Bloemendaal/Original_" + dir_name + "_" + Description + ".csv")
            ...
            if Image_Name in DF.values:
                ...
                temp = pd.DataFrame([data], columns=columns)
                merged = pd.concat([temp, DF])
                merged.to_csv("Output data/Bloemendaal/Original_" + dir_name + "_" + Description + ".csv", index=False)

        except FileNotFoundError:
            temp = pd.DataFrame([data], columns=[columns])
            temp.to_csv("Output data/Bloemendaal/Original_" + dir_name + "_" + Description + ".csv", index=False)
...
path_of_the_directory = os.path.join('/home/casper/Documents/Aardwetenschappen/MSc Thesis/Photo/Bloemendaal/', input_directory)
dir_path = os.path.dirname(path_of_the_directory)
dir_name = os.path.basename(dir_path)
